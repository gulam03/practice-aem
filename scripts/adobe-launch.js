...
class AdobeLaunch extends TagManager {
  setEndpoint() {
    if (!window.placeholders.default.launchProductionUrl || !window.placeholders.default.launchNonProductionUrl) {
      throw Error('missing placeholders for analytics endpoint');
    }
    if (window.location.hostname.includes('--') || window.location.hostname === 'localhost') {
